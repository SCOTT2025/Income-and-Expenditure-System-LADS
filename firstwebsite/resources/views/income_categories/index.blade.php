...
<div class="container-fluid">
    <h3 class="mb-4">Income Category List</h3>

    <div class="d-flex mb-3 gap-2">
        <a href="{{ route('income_categories.create') }}" class="btn btn-success">Add Income Category</a>
                ...
                <tbody>
                    @foreach($incomeCategories as $category)
                        <tr>
                            ...
                            <td>{{ $category->name }}</td>
                            <td>
                                <a href="{{ route('income_categories.show', $category->id) }}" class="btn btn-sm btn-primary">View</a>
                                <a href="{{ route('income_categories.edit', $category->id) }}" class="btn btn-sm btn-info">Edit</a>
                                <form action="{{ route('income_categories.destroy', $category->id) }}" method="POST" class="d-inline">
                                    @csrf
                                    @method('DELETE')
    ...
    $(function () {
        ...
        $('#bulk-delete').on('click', function () {
            ...
            if (confirm('Are you sure to delete selected categories?')) {
                $.ajax({
                    url: '{{ route("income_categories.bulkDelete") }}',
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        ids: selected
